...
public class Main {
    ...
    public static void main(String[] args) throws FileNotFoundException{
        ...
        while(t!=0){
            ...
            lll.clear();
            for(Link li: ll){
                if(li.getSgroup().equals(key)) lll.add(li);
            }
            
            lll.sort((Link a, Link b)->{
                if(a.getSbjcode().equals(b.getSbjcode())){
                    return a.getScode().compareTo(b.getScode());
                }
                return a.getSbjcode().compareTo(b.getSbjcode());
            });
            
            System.out.println("BANG DIEM lop "+key+":");
            for(Link li: lll){
